...
/// </remarks>
public sealed class AzurePrepDbSeederTests : IDisposable
{
    private readonly SqliteConnection _connection;

    public AzurePrepDbSeederTests()
    {
    ...
    public async Task Semear_CriaTodosOsExamesDefinidos()
    {
        ...
        Assert.Equal(
            AzurePrepDbSeeder.Exames.Select(e => e.Code).OrderBy(c => c),
            codigos.OrderBy(c => c));
    }
    ...
    public async Task Semear_GravaAreasComOsPesosDaDefinicao()
    {
        ...
        var definicao = AzurePrepDbSeeder.Exames[0];
    ...
    public async Task Semear_DuasVezes_NaoDuplicaExameNemArea()
    {
        ...
        Assert.Equal(AzurePrepDbSeeder.Exames.Count, await leitura.Exams.CountAsync());
        Assert.Equal(
            AzurePrepDbSeeder.Exames.Sum(e => e.Areas.Count),
    ...
    public void Validar_LoteDeExameInexistente_ImpedeOSeed()
    {
        ...
        var problemas = CatalogoDeQuestoesDeSeed.Validar([lote], AzurePrepDbSeeder.AreasPorExame);
