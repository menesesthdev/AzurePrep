...
- **`Validar_CatalogoRealEmbutido_EstaIntegro`** roda a validação sobre os arquivos de verdade, contra as áreas de verdade (`AzurePrepDbSeeder.SlugsDeArea`, público só para isso). Antes, os testes de regra usavam lotes sintéticos e nunca tocavam nos JSONs; a única checagem do catálogo real era o seed derrubando a aplicação no startup — ou seja, o erro aparecia no deploy, não no commit.
